...
def get_aggregated_data(reviews_df):
    start = time.time()
    client = Client()
    client = client.restart()
    print(client)

    user_reviews = dd.read_csv(reviews_df, dtype=dtypes)
    ...
    with open('ground_truth_results.json', 'w') as outfile: json.dump(json.loads(submit.to_json()), outfile)
    return runtime


if __name__ == '__main__':
    print('PyCharm')
    get_aggregated_data("user_reviews_test_small.csv")
